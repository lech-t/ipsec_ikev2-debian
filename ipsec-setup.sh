...
# by Lech Taczkowski
#------------------------------------------------------------------------------
#< Description:   This script is useful for setting up IPSec server with
#                 Strongswan + IKEv2 + key authentication (Roadwarrior).
#		  It allows users to get secure access to another network over
#		  an unsecure network (Internet). It also routes Internet
#		  traffic through the tunnel.
#
#		  It has been written for my own personal use for the following
#		  setup, where the VPN server is behind a router and is NATted.
#		  The ports are being redirected. 
#		  There are so many possible scenarios of setting up StrongSwan
#		  with IKEv2. This script is only for one of them.
#
